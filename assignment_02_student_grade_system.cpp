...
char getGrade(int score) {
    // Input validation: check if score is out of range [0, 100]
    if (score < 0 || score > 100) {
        return '\0'; // Null character indicates an error
    }
    
    // Determine the grade using conditional checks
    if (score >= 80) {
        return 'A';
    } else if (score >= 70) {
        return 'B';
...
int main() {
    ...
    // If getGrade returned '\0', handle as an invalid score
    if (grade == '\0') {
